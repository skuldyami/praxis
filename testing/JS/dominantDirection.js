...
function getScript(char){
  const code= char.normalize().codePointAt(0) //normalizing NCF(composition)
  let index= SCRIPTS.findIndex(script=>{
    for(let i=0; i<script.ranges.length; i++){
      if((code>= script.ranges[i][0]) && (code< script.ranges[i][1])){
        return true;
      }
    }
    return false;
...
function dominantDirection(text){
  let w_direction=SCRIPTS.map(arr=>arr.direction);
  let w_dir_dict={};
  let max=-1;
  let tempKey;
  for(elem of w_direction){ //fill dict keys
    w_dir_dict[elem]=0;
  }
  for(elem of text){ //fill dict values
    let index= getScript(elem);
    if(index != -1){
      w_dir_dict[SCRIPTS[index].direction]+=1;
    }
  }
  console.log(w_dir_dict);
  for(key of Object.keys(w_dir_dict)){
    if(w_dir_dict[key]>max){
      max=w_dir_dict[key];
      tempKey=key;
    }
  }
...
let s3="Espléndido";
let s4="Hey, مساء الخير";
let s5="𐡀";
let s6="ᠡ‍᠊ᠡ‍ᠸ‍ᠷ‍ᡁ‍hey᠊ᠠ‍ᠢ";
let s7="分かりました";
console.log(dominantDirection(s3));
console.log(dominantDirection(s4));
console.log(dominantDirection(s5));
console.log(dominantDirection(s6));
console.log(dominantDirection(s7));